---
title: "Predictions of Exercise Pattern Using Weight Lifting Exercises Dataset"
...
This project is based on the dataset provided by [http://groupware.les.inf.puc-rio.br/har](http://groupware.les.inf.puc-rio.br/har). This project aims to train a predictive model to predict what exercise was performed using a dataset with 159 features

We will follow these steps:

- Process the data to use in this project
- Explore the data and focus on the two parameters of interest
- Model selection and try different model to find out the answers to our questions
- Model examination, to check wether our best model holds up to our standards
- Draw a Conclusion to we answer the questions based on the data
- Predict the classification of the model on the test set
...
Let's remove the data with NA / empty values
...
We see that some features are quite correlated with each other. We will create a model with these excluded. Then, we'll try and reduce the features by running PCA on all and the excluded subset of the features
...
This concludes that nor PCA doesn't have a positive accuracy (or the process time for that matter)
...
The predictions don't change a lot with each model, but since we have the most faith in the `rfMod.exclude`, we'll keep that as the final answer. 
